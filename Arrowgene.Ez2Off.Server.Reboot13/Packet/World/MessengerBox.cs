...
namespace Arrowgene.Ez2Off.Server.Reboot13.Packet.World
{
    public class MessengerBox : Handler<EzServer>
    {
        ...
        public override void Handle(EzClient client, EzPacket packet)
        {
            ...
            int index = packet.Data.ReadInt32();
            ...
            if (action == MessageBoxActionType.View)
            {
                List<Message> messages = client.MessageBox.GetMessages();
                Router.Send(client, 62, MessagePacket.CreateMessageBox(messages));
            }

            if (action == MessageBoxActionType.Delete)
            {
                Message message = client.MessageBox.GetMessage(index);
            ...
            if (action == MessageBoxActionType.Send)
            {
                ...
                EzClient receiverClient = client.Channel.GetClient(receiver);
                ...
                else
                {
                    ...
                    Logger.Debug(client, $"Character: {receiver} offline, save MessageId: {message.Id} to database");
