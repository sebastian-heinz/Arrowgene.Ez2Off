...
namespace Arrowgene.Ez2Off.Server.Database.Sql
{
    ...
    {
        private static readonly ILogger _logger = LogProvider.Logger(typeof(MariaDb));

        public static void Drop(string host, short port, string user, string password, string database)
        {
            try
            {
        ...
        public MariaDb(string host, short port, string user, string password, string database)
        {
        ...
        public bool SetSetting(string key, string value)
        {
            ...
            try
            {
                Upsert("setting", columns, values, out int id);
            }
            catch (Exception exception)
            {
                _logger.Exception(exception);
        ...
        public bool UpsertSetting(Setting setting)
        {
            ...
            try
            {
                Upsert("ez2on_setting", columns, values, out int id);
            }
            catch (Exception exception)
            {
                _logger.Exception(exception);
        ...
        public bool UpsertCharacter(Character character)
        {
            ...
            try
            {
                Upsert("ez2on_character", columns, values, out int id);
            }
            catch (Exception exception)
            {
                _logger.Exception(exception);
        ...
        public bool UpsertItem(Item item)
        {
            ...
            string[] values =
            {
                ...
                $"{item.ResiliencePlus}", $"{item.DefensePlus}", $"{(int) item.Type}", $"{(item.Enabled ? 1 : 0)}",
                $"{(int) item.Currency}"
            };
            try
            {
                Upsert("ez2on_item", columns, values, out int id);
            }
            catch (Exception exception)
            {
                _logger.Exception(exception);
        ...
        public bool UpsertQuest(Quest quest)
        {
            ...
            try
            {
                Upsert("ez2on_quest", columns, values, out int id);
            }
            catch (Exception exception)
            {
                _logger.Exception(exception);
        ...
        public bool UpsertSong(Song song)
        {
            ...
            string[] values =
            {
                $"{song.Id}", $"'{Escape(song.Name)}'", $"{(int) song.Category}", $"'{song.Duration}'", $"{song.Bpm}",
                $"'{song.FileName}'",
            ...
            try
            {
                Upsert("ez2on_song", columns, values, out int id);
            }
            catch (Exception exception)
            {
                _logger.Exception(exception);
        ...
        public bool UpsertRadiomix(Radiomix radiomix)
        {
            ...
            string[] values =
            {
                ...
                $"{radiomix.Song1Id}", $"{radiomix.Song1RubyNotes}", $"{radiomix.Song1StreetNotes}",
                $"{radiomix.Song1ClubNotes}",
                $"{radiomix.Song1Club8KNotes}",
                $"{radiomix.Song2Id}", $"{radiomix.Song2RubyNotes}", $"{radiomix.Song2StreetNotes}",
                $"{radiomix.Song2ClubNotes}",
                $"{radiomix.Song2Club8KNotes}",
                $"{radiomix.Song3Id}", $"{radiomix.Song3RubyNotes}", $"{radiomix.Song3StreetNotes}",
                $"{radiomix.Song3ClubNotes}",
                $"{radiomix.Song3Club8KNotes}",
                $"{radiomix.Song4Id}", $"{radiomix.Song4RubyNotes}", $"{radiomix.Song4StreetNotes}",
                $"{radiomix.Song4ClubNotes}",
                $"{radiomix.Song4Club8KNotes}",
            };
            try
            {
                Upsert("ez2on_radiomix", columns, values, out int id);
            }
            catch (Exception exception)
            {
                _logger.Exception(exception);
        ...
        private List<int> SelectBestScoreIds(int songId, ModeType mode, DifficultyType difficulty, int scoreCount = -1)
        {
            ...
            string sql = String.Join(Separator,
                "SELECT `ez2on_score`.`character_id`, MAX(`ez2on_score`.`total_score`) as `max_total_score` FROM `ez2on_score`",
                $"WHERE `ez2on_score`.`song_id`={songId} AND `ez2on_score`.`mode`={(int) mode}",
                $"AND `ez2on_score`.`difficulty`={(int) difficulty} AND `ez2on_score`.`incident`={0}",
                "GROUP BY `ez2on_score`.`character_id`",
                "ORDER BY `max_total_score` DESC, `ez2on_score`.`created` ASC"
            );
            if (scoreCount > 0)
            {
            ...
            string sqlIds = String.Join(Separator,
                "SELECT `ez2on_score`.`id`",
                "FROM `ez2on_score`",
                $"WHERE `ez2on_score`.`song_id`={songId} AND `ez2on_score`.`mode`={(int) mode}",
                $"AND `ez2on_score`.`difficulty`={(int) difficulty} AND `ez2on_score`.`incident`={0}",
                "AND (`ez2on_score`.`character_id`, `ez2on_score`.`total_score`) IN ("
            );

            for (int i = 0; i < results.Count; i++)
            {
                Tuple<int, int> result = results[i];
                sqlIds += $"({result.Item1}, {result.Item2})";
            ...
            sqlIds += ") ORDER BY `total_score` DESC, `ez2on_score`.`created` ASC;";

            ExecuteReader(sqlIds, reader =>
            {
                while (reader.Read())
                {
        ...
        public Score SelectBestScore(int characterId, int songId, ModeType mode, DifficultyType difficulty)
        {
            string sql = String.Join(Separator,
                ...
                $"AND `ez2on_score`.`mode`={(int) mode} AND `ez2on_score`.`difficulty`={(int) difficulty}",
        ...
        public bool InsertMessage(Message message)
        {
            ...
            int affectedRows = ExecuteNonQuery(sql, out int autoIncrement);
        ...
        public bool InsertScoreIncident(Incident incident, int scoreId)
        {
            ...
            string sqlCreate = String.Join(Separator,
                ...
                $"{scoreId},",
                $"{incident.Id}",
                ") ON DUPLICATE KEY UPDATE `score_id` = `score_id`;"
            );
            int affectedRows = ExecuteNonQuery(sqlCreate, out int autoIncrement);
            return affectedRows > 0;
        }
        ...
        public bool InsertScoreIncident(int incidentId, int scoreId)
        {
            ...
            {
                _logger.Exception(ex);
                _logger.Error($"code:{ex.ErrorCode} nr:{ex.Number}");
            }
        ...
        public bool InsertIdentification(Identification identification)
        {
            ...
            int affectedRows = ExecuteNonQuery(sqlCreate, out int autoIncrement);
            identification.Id = autoIncrement;
            return affectedRows > 0;
        }
        ...
        private int ExecuteNonQuery(string sql, out int autoIncrement)
        {
            ...
            using (MySqlConnection connection = NewConnection())
            {
                using (MySqlCommand command = connection.CreateCommand())
                {
                    command.CommandText = sql;
                    rows = command.ExecuteNonQuery();
                    autoIncrement = GetAutoIncrement(command);
                }
            }
        ...
        private int Upsert(string table, string[] columns, string[] values, out int autoIncrement)
        {
            int rowsAffected;
            using (MySqlConnection connection = NewConnection())
            {
                ...
                using (MySqlCommand command = connection.CreateCommand())
                {
                    ...
                    for (int i = 0; i < length; i++)
                    {
                        ...
                        updateBuilder.Append($"`{column}`=VALUES(`{column}`)");
                        if (i < length - 1)
                        {
                            query.Append(", ");
        ...
        private int GetAutoIncrement(MySqlCommand command)
        {
