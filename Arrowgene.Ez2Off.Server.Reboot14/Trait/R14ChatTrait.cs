...
{
    public class R14ChatTrait : ChatTrait
    {
        public R14ChatTrait(EzServer server) : base(server)
        {
        }

        public override void SendChannel(ChatMessage message)
        {
            IBuffer packet = PacketBuilder.ChatPacket.CreateChannel(message.SenderName, message.Message);
        ...
        public override void SendRoom(ChatMessage message)
        {
            ...
            IBuffer packet = PacketBuilder.ChatPacket.CreateRoom(slot, message.SenderName, message.Message);
            Router.Send(message.Recipients, 18, packet);
        }

        public override void SendWhisper(ChatMessage message)
        {
            IBuffer packet = PacketBuilder.ChatPacket.CreateWhisper(message.SenderName, message.Message);
        ...
        public override void SendDirect(ChatMessage message)
        {
            ...
            Router.Send(
                ...
                PacketBuilder.ChatPacket.CreateDirect(
                    message.SenderName,
        ...
        {
            IBuffer packet = PacketBuilder.ChatPacket.CreateGm(message.Message);
